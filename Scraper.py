...
class Scraper:
            ...
            with open(f"json/{file_read_path.format(f)}", "r") as fr:
                with open(f"test_dir/{file_write_path.format(f)}", "a") as fw :
                    ...
                    for x in range(int(data['pageItemCount'])):
                        ...
                        video_url, video_size = self.fetch_raw_mp4(x, url)
    ...
    def fetch_raw_mp4(self, count, url):
        ...
        if data != None:
            ...
            try:
                if json_result != None:
                    result = json_result[0][1]['data']['widgets'][0]['mediaCollection']['embedded']['streams'][0]['media'][0]['url']
